  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8 font-sans text-stone-900">
      ...
      <div className="admin-card p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {(Object.keys(THEME_FONTS) as ThemeFontType[]).map((fontKey) => {
              ...
              const previewFontClass = getThemeFontClass(fontKey)
              return (
                <button
                  key={fontKey}
                  type="button"
                ...
                >
                  <div className={`text-base font-bold text-stone-900 mb-0.5 ${previewFontClass}`}>
                    {option.name}
                  </div>
                  <div className="text-xs text-stone-500 mb-2">{option.description}</div>
                  <div className={`text-xs text-orange-900 font-medium ${previewFontClass}`}>
        ...
        {/* Prévia ao Vivo do Cardápio do Cliente (Live Preview) */}
        <div className="rounded-2xl border border-stone-200 bg-stone-50/80 p-4 sm:p-5 space-y-3.5">
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold uppercase tracking-wider text-stone-700">
                Prévia ao Vivo do Cardápio (Visualização do Cliente)
              </span>
            </div>
            {slugValue && (
              <a
                ...
                className="inline-flex items-center gap-1 text-xs font-semibold text-orange-700 hover:text-orange-800 transition-colors"
              >
                Abrir cardápio real
          ...
          {/* Mini Cardápio Interativo com Tipografia e Cores em Tempo Real */}
          <div className={`rounded-xl overflow-hidden shadow-xs border border-stone-200/90 bg-white ${getThemeFontClass(themeFontValue)}`}>
            {/* Cabeçalho simulado */}
            <div
              className="p-4 sm:p-5 text-center transition-colors duration-200"
              style={{ backgroundColor: themeColorValue, color: contrastColor }}
            >
              <h3 className="text-lg sm:text-xl font-bold tracking-tight">
                {nameValue || restaurant.name || 'Nome do Restaurante'}
              </h3>
              <p className="text-xs opacity-90 mt-1 max-w-sm mx-auto font-normal">
                {taglineValue || restaurant.tagline || 'Cozinha artesanal, cortes nobres e chopp gelado'}
              </p>
            </div>

            {/* Simulação de Item no Cardápio */}
            <div className="p-4 space-y-3 bg-stone-50/50">
              <div className="flex items-center justify-between text-xs font-semibold text-stone-500 border-b border-stone-200/60 pb-2">
                <span className="uppercase tracking-wider">Pratos Principais</span>
                <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-white border border-stone-200 text-stone-600">
                  Tipografia: {THEME_FONTS[themeFontValue as ThemeFontType]?.name || 'Moderno'}
                </span>
              </div>

              <div className="flex items-start justify-between gap-3 p-3 rounded-xl bg-white border border-stone-200 shadow-2xs">
                <div className="min-w-0">
                  <h4 className="text-sm sm:text-base font-bold text-stone-900 leading-snug">
                    Picanha na Brasa com Farofa & Fritas
                  </h4>
                  <p className="text-xs text-stone-500 mt-1 line-clamp-1">
                    Corte selecionado grelhado ao ponto, acompanhado de chimichurri da casa.
                  </p>
                </div>
                <span className="text-sm font-bold text-stone-900 tabular-nums shrink-0 whitespace-nowrap">
                  R$ 68,00
                </span>
              </div>
            </div>
          ...
          <p className="text-[11px] text-stone-500 leading-relaxed">
            💡 <strong>Onde isso se aplica?</strong> Essa família tipográfica e cor são aplicadas no <strong>cardápio digital dos seus clientes</strong> (acessado via QR Code ou link). O seu painel de controle administrativo mantém a fonte padrão de sistema para garantir agilidade e clareza na gestão dos dados.
          </p>
        </div>
